...
# auto_update
@scheduler.scheduled_job("cron", hour='8',minute='5',id="auto_update")
...
# auto_today
@scheduler.scheduled_job("cron", hour='8',minute='5',id="auto_today")
...
async def get_today():
    ...
    if cnt_cf+cnt_atc+cnt_nowcoder==0: return '今天没有比赛,但也要好好做题哦=￣ω￣= ——>hple查看帮助'
    else:
        if cnt_cf!=0:
            ans=ans+"\n"+get_cf
        if cnt_atc!=0:
